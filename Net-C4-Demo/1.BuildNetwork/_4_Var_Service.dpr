...
begin
  ...
  // 接通调度端,文件服务,用户数据库服务
  PasAI.Net.C4.C40_PhysicsTunnelPool.GetOrCreatePhysicsTunnel(Internet_DP_Addr_, Internet_DP_Port_, 'dp|FS|UserDB', TMonitorMySAAS.Create);

  // 循环检查指定端是否准备就绪,便于我们触发某些事件
  PasAI.Net.C4.C40_ClientPool.WaitConnectedDoneP('dp|FS|userDB', procedure(States_: TC40_Custom_ClientPool_Wait_States)
    begin
