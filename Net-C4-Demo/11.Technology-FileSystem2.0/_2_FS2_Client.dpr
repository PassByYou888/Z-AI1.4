...
begin
  case CtrlType of
    CTRL_C_EVENT, CTRL_BREAK_EVENT, CTRL_CLOSE_EVENT, CTRL_LOGOFF_EVENT, CTRL_SHUTDOWN_EVENT:
      begin
        TCompute.SyncC(PasAI.Net.C4.C40Clean);
      end;
  end;
  Result := True;
end;
...
begin
  ...
  PasAI.Net.C4.C40_PhysicsTunnelPool.GetOrCreatePhysicsTunnel(Internet_DP_Addr_, Internet_DP_Port_, 'DP|FS2', nil);

  // FS也是一种主要基础设施，主要用于存放大型数据，例如图片，列表，配置数据等等
  // C4的FS支持高频率擦写，VM服务器可以用FS作为数据交换，但要区分Var网络变量
  PasAI.Net.C4.C40_ClientPool.WaitConnectedDoneP('FS2', procedure(States_: TC40_Custom_ClientPool_Wait_States)
    var
      tmp1, tmp2: TMS64;
    begin
