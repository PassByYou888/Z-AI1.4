...
begin
  ...
  // 因为sha3就是抗力量子设计，而安全的密码则保证了多次Rijndael加密是安全的
  // 在双重安全机制下，Encrypt方法可以抵御未来的量子攻击
  PasAI.Cipher.QuantumEncrypt(sour, m64, 1024, TPascalString('password123456').Bytes);

  sour.Clear;
  m64.Position := 0;
  // 在不知道密码的情况下，m64中的数据永远不可能被解密出来
  if PasAI.Cipher.QuantumDecrypt(m64, sour, TPascalString('password123456').Bytes) then
      DoStatus('成功解密')
  else
      DoStatus('密码错误');
