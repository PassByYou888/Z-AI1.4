...
uses
  ...
  PasAI.Core, PasAI.ZDB.Engine, PasAI.ZDB.LocalManager, PasAI.Status, PasAI.DFE, PasAI.PascalStrings,
  PasAI.ListEngine, PasAI.UnicodeMixedLib, PasAI.MemoryStream, PasAI.Expression, PasAI.OpCode, PasAI.Net.XNAT.Physics, PasAI.Notify,
...
type
  ...
  public
    ...
    procedure cmd_TestBigStream(Sender: TPeerClient; InData: TCore_Stream; BigStreamTotal, BigStreamCompleteSize: Int64);
  ...
  TChatClient = class(TZNet_DoubleTunnelClient_NoAuth)
  public
  ...
  TForm3 = class(TForm, IMsgNotify, IZNet_VMInterface)
    Layout1: TLayout;
    Label1: TLabel;
    HostEdit: TEdit;
  ...
  public
    ...
    phyServer: TZNet_Server;
    phyClient: TZNet_Client;
    serv: TChatServer;
    cli: TChatClient;
...
constructor TChatServer.Create(ARecvTunnel, ASendTunnel: TZNet_Server);
begin
...
procedure TChatServer.cmd_TestBigStream(Sender: TPeerClient; InData: TCore_Stream; BigStreamTotal, BigStreamCompleteSize: Int64);
var
  MyUserSpec: TChatServer_UserSpecial;
begin
  // 在双通道的编程中，都要在指令前面加上linkOK的判断
...
constructor TChatClient.Create(ARecvTunnel, ASendTunnel: TZNet_Client);
begin
  inherited Create(ARecvTunnel, ASendTunnel);
end;
...
begin
  ...
  serv := TChatServer.Create(TZNet_WithP2PVM_Server.Create, TZNet_WithP2PVM_Server.Create);
  serv.RecvTunnel.StartService('::', 0);
  serv.SendTunnel.StartService('::', 1);
  serv.RegisterCommand;

  cli := TChatClient.Create(TZNet_WithP2PVM_Client.Create, TZNet_WithP2PVM_Client.Create);
  cli.MsgNotify := self;
  cli.RegisterCommand;
end;
...
begin
  if TPascalString(TButton(Sender).Text).StrExists('stop') then
    begin
