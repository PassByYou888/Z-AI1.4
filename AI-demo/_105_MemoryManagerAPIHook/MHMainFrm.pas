...
begin
  ...
  // 这里我们会发现泄漏
  DoStatus('leakproc函数分配了 %d 字节的内存', [PasAI.MH.GetHookMemorySize_1]);
...
begin
  ...
  // 这里我们会发现泄漏
  DoStatus('TMyRec总分分配了 %d 次内存，占用 %d 字节空间，', [PasAI.MH.GetHookPtrList_1.Count, PasAI.MH.GetHookMemorySize_1]);
...
begin
  ...
  DoStatus('总共内存分配 %d 次 占用 %s 空间，地址跨度为：%s ', [PasAI.MH3.GetHookPtrList.Count, umlSizeToStr(PasAI.MH3.GetHookMemorySize).Text,
