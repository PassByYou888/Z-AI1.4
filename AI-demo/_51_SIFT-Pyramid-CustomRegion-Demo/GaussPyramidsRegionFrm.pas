...
uses
  ...
  PasAI.UnicodeMixedLib, PasAI.Learn, PasAI.Learn.Type_LIB, PasAI.Learn.SIFT, PasAI.Geometry2D,

  PasAI.DrawEngine, PasAI.DrawEngine.SlowFMX, FMX.ExtCtrls, FMX.Memo.Types;
...
begin
  ...
  ft1.LinkRaster := ort1;
  DisposeObject(t1);
  t1 := ft1.CreateFeatureViewer((ort1.width + ort1.Height) * 0.5 * 0.005, PasAI_RasterColorF(1, 0, 0, 0.2)) as TDETexture;
end;
...
begin
  ...
  ft2.LinkRaster := ort2;
  DisposeObject(t2);
  t2 := ft2.CreateFeatureViewer((ort2.width + ort2.Height) * 0.5 * 0.005, PasAI_RasterColorF(1, 1, 0, 0.2)) as TDETexture;
end;
