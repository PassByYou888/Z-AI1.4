...
uses
  ...
  PasAI.Core, PasAI.PascalStrings, PasAI.UnicodeMixedLib, PasAI.Geometry2D, PasAI.MemoryRaster,
  PasAI.MemoryStream, PasAI.Status, PasAI.DrawEngine, PasAI.DrawEngine.FMX;
...
type
  ...
  public
    ...
    procedure BuildGeometry(w, h, steps: Integer; output: TMPasAI_Raster);
  end;
...
procedure TFillColorForm.BuildGeometry(w, h, steps: Integer; output: TMPasAI_Raster);
const
  edge = 30;
var
  j: Integer;
...
begin
  ...
  DelphiParallelFor(5, True, 0, h - 1, procedure(pass: Integer)
    var
      i: Integer;
    begin
      ...
      for i := 0 to w - 1 do
        if vl.InHere(Vec2(i, pass)) then
            output.Pixel[i, pass] := PasAI_RasterColorF(0.5, 0, 0, 1);
    end);
  // 三角填充，这种方法是内置的，多用于直接画图
  // output.Vertex.FillPoly(vl, RasterColorF(0, 1, 0, 0.5));

  // 画包围线，填充法只会在指定背景色边缘画轮廓
  output.FillNoneBGColorBorder(PasAI_RasterColorF(0, 0, 0, 1), RColorF(1, 1, 1, 1), 4);

  // 画重心轴
  output.DrawCrossF(vl.Centroid, 10, PasAI_RasterColorF(1, 1, 1, 1));
