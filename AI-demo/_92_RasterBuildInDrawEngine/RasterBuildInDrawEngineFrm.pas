...
uses
  ...
  PasAI.MemoryRaster, PasAI.DrawEngine, PasAI.DrawEngine.SlowFMX, PasAI.Geometry2D, PasAI.DrawEngine.PictureViewer;
...
function BuildDemoPicture(var angle: TGeoFloat; deltaTime: Double): TPasAI_Raster;
...
begin
  ...
  with Result.DrawEngine do
    begin
      for i := 0 to PasAI_Raster_.TextCoordinate.Count - 1 do
        begin
          DrawBox(PasAI_Raster_.TextCoordinate[i].BoundBox, DEColor(1, 0, 0), 1);
          DrawBox(PasAI_Raster_.TextCoordinate[i].BoundBox.BoundRect, DEColor(1, 1, 1), 1);
        end;
      Flush;
    end;
end;
