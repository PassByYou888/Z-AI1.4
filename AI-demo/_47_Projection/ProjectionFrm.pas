...
uses
  ...
  PasAI.Geometry2D,
  PasAI.DrawEngine, PasAI.MemoryRaster, PasAI.DrawEngine.SlowFMX, FMX.Layouts, FMX.Controls.Presentation, FMX.StdCtrls, FMX.ListBox;
...
begin
  ori := NewPasAI_RasterFromFile(umlCombineFileName(TPath.GetLibraryPath, 'canglaoshi.bmp'));
  ori.Black();

  sour := NewPasAI_Raster();
  sour.Assign(ori);
  ...
  dest.SetSize(round(d_Image.Width), round(d_Image.Height), PasAI_RasterColor(0, 0, 0));
...
begin
  ...
  if TriangleCheckBox.IsChecked then
    begin
      ...
      TPasAI_RasterVertex.DebugTriangle := True;
      TPasAI_RasterVertex.DebugTriangleColor := RColorF(1.0, 0.5, 0.5, 1);
    end
  else
    begin
      dest.CloseAgg;
  ...
  sour.DrawRect(ps, PasAI_RasterColorF(1, 0.5, 0.5));
  MemoryBitmapToBitmap(sour, s_Image.Bitmap);
end;
