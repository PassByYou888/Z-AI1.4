...
uses
  ...
  PasAI.Net.C4_NetDisk_Service, PasAI.Net.C4_NetDisk_Client, PasAI.Net.C4_NetDisk_Directory, PasAI.Net.C4_NetDisk_Admin_Tool,
...
begin
  with PasAI.Net.C4.TC40_PhysicsService.Create(ServListeningIPEdit.Text,
    ServIPEdit.Text, EStrToInt(ServPortEdit.Text, 0), PasAI.Net.PhysicsIO.TPhysicsServer.Create) do
    begin
...
begin
  ...
  PhysicsReconnectionDelayEdit.Text := umlShortFloatToStr(PasAI.Net.C4.C40_PhysicsReconnectionDelayTime);
  UpdateServiceInfoTimerEdit.Text := umlIntToStr(PasAI.Net.C4.C40_UpdateServiceInfoDelayTime);
...
begin
  ...
  PasAI.Net.C4.C40_SafeCheckTime := EStrToInt(SafeCheckTimerEdit.Text, PasAI.Net.C4.C40_SafeCheckTime);
  PasAI.Net.C4.C40_PhysicsReconnectionDelayTime := EStrToDouble(PhysicsReconnectionDelayEdit.Text, PasAI.Net.C4.C40_PhysicsReconnectionDelayTime);
  PasAI.Net.C4.C40_UpdateServiceInfoDelayTime := EStrToInt(UpdateServiceInfoTimerEdit.Text, PasAI.Net.C4.C40_UpdateServiceInfoDelayTime);
  PasAI.Net.C4.C40_PhysicsServiceTimeout := EStrToInt(PhysicsServiceTimeoutEdit.Text, PasAI.Net.C4.C40_PhysicsServiceTimeout);
  PasAI.Net.C4.C40_PhysicsTunnelTimeout := EStrToInt(PhysicsTunnelTimeoutEdit.Text, PasAI.Net.C4.C40_PhysicsTunnelTimeout);
  PasAI.Net.C4.C40_KillIDCFaultTimeout := EStrToInt(KillIDCFaultTimeoutEdit.Text, PasAI.Net.C4.C40_KillIDCFaultTimeout);
...
begin
  ...
  for i := 0 to C40_PhysicsTunnelPool.Count - 1 do
    begin
      phy_tunnel := C40_PhysicsTunnelPool[i];
      dpc_arry := phy_tunnel.DependNetworkClientPool.SearchClass(PasAI.Net.C4.TC40_Dispatch_Client, True);
      for j := 0 to length(dpc_arry) - 1 do
          L.MergeAndUpdateWorkload(PasAI.Net.C4.TC40_Dispatch_Client(dpc_arry[j]).Service_Info_Pool);
    end;
  for i := 0 to C40_PhysicsServicePool.Count - 1 do
    begin
      phy_serv := C40_PhysicsServicePool[i];
      dps_arry := phy_serv.DependNetworkServicePool.GetFromClass(PasAI.Net.C4.TC40_Dispatch_Service);
      for j := 0 to length(dps_arry) - 1 do
          L.MergeAndUpdateWorkload(PasAI.Net.C4.TC40_Dispatch_Service(dps_arry[j]).Service_Info_Pool);
    end;
  for i := 0 to L.Count - 1 do
    begin
      ...
      begin
        if cs.ConfigIsUpdate then
          begin
            ...
            PasAI.Net.C4.C40_RootPath := cs.Config.GetDefaultValue('Root', PasAI.Net.C4.C40_RootPath);
            if not umlDirectoryExists(PasAI.Net.C4.C40_RootPath) then
                umlCreateDirectory(PasAI.Net.C4.C40_RootPath);
            PasAI.Net.C4.C40_Password := cs.Config.GetDefaultValue('Password', PasAI.Net.C4.C40_Password);
