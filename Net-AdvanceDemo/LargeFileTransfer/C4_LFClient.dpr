...
begin
  ...
  if Is_Download then
    begin
      if To_Dest_File.L = 0 then
          To_Dest_File := umlCombineFileName(PasAI.Net.C4.C40_RootPath, Download_File);
    end;
...
begin
  ...
  DisposeObject(FS);
  LMD5 := m64.ToMD5;
  m64.SaveToFile(umlCombineFileName(PasAI.Net.C4.C40_RootPath, PFormat('%s.cache', [umlMD5ToStr(LMD5).Text])));
...
begin
  ...
  DisposeObject(d);
  m64.SaveToFile(umlCombineFileName(PasAI.Net.C4.C40_RootPath, PFormat('%s.cache', [umlMD5ToStr(m64.ToMD5).Text])));
...
begin
  inherited Run();
  cache_f := umlCombineFileName(PasAI.Net.C4.C40_RootPath, PFormat('%s.cache', [umlMD5ToStr(LMD5).Text]));
...
begin
  if Fill_CMD then
    begin
      ...
      PasAI.Net.C4.C40_QuietMode := True;
      ...
      if PasAI.Net.C4_Console_APP.C40_Extract_CmdLine then
        begin
          PasAI.Net.C4.C40_ClientPool.WaitConnectedDoneC('FS2', @Do_Wait_FS2_Connected_Done);
