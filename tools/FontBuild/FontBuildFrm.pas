...
uses
  ...
  PasAI.MemoryStream, PasAI.MemoryRaster, PasAI.Geometry2D, PasAI.Status, PasAI.PascalStrings, PasAI.UPascalStrings, PasAI.Core, GR32, PasAI.DrawEngine;
...
procedure TFontBuildForm.BuildButtonClick(Sender: TObject);
  ...
  function computeYHead(R: TMPasAI_Raster): Integer;
  var
    X, Y: Integer;
  begin
  ...
  function computeYTail(R: TMPasAI_Raster; sp: Integer): Integer;
  var
    X, Y: Integer;
  begin
  ...
  begin
    ...
    if (GR.width > 0) and (GR.height > 0) then
      begin
        ...
        N2 := TMPasAI_Raster.Create;
        N2.DrawMode := PasAI.MemoryRaster.TDrawMode.dmBlend;
        ...
        if EX - BX > 0 then
          begin
            n := TMPasAI_Raster.Create;
            n.SetSize(EX - BX, GR.height, PasAI_RasterColor(0, 0, 0, 0));
...
begin
  if not bmpSaveDialog.Execute then
      Exit;
  m64 := TMS64.Create;
  FR.ExportPasAI_Raster(m64, False);
...
begin
  ...
  raster.DrawMode := PasAI.MemoryRaster.TDrawMode.dmBlend;
