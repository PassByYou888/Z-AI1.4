...
uses
  ...
  PasAI.Notify, PasAI.Cadencer,
  PasAI.TextDataEngine, PasAI.ListEngine,
  PasAI.DrawEngine.SlowFMX, PasAI.DrawEngine, PasAI.Geometry2D, PasAI.Geometry3D,
  PasAI.MemoryStream,
  PasAI.Learn, PasAI.Learn.Type_LIB,
  PasAI.ZAI.Common, PasAI.ZAI,
...
type
  ...
  public
    ...
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure SetInputRaster(r: TMPasAI_Raster);
...
function OpenSamplerEditor(Bounds: TRect; Input: TMPasAI_Raster): TSampleEditorForm; overload;
function OpenSamplerEditor(Input: TMPasAI_Raster): TSampleEditorForm; overload;
...
function OpenSamplerEditor(Bounds: TRect; Input: TMPasAI_Raster): TSampleEditorForm;
var
  f: TSampleEditorForm;
begin
...
function OpenSamplerEditor(Input: TMPasAI_Raster): TSampleEditorForm;
var
  f: TSampleEditorForm;
begin
  f := TSampleEditorForm.CustomCreate(nil, False);
...
begin
  if not SavePictureDialog.Execute then
      exit;
  PasAI_Raster_ := ViewIntf.Items[0].Raster;
  if umlMultipleMatch('*.bmp', SavePictureDialog.FileName) then
      PasAI_Raster_.SaveToBmp24File(SavePictureDialog.FileName)
  else if umlMultipleMatch('*.jpg', SavePictureDialog.FileName) then
      PasAI_Raster_.SaveToJpegYCbCrFile(SavePictureDialog.FileName, 90)
  else if umlMultipleMatch('*.jls', SavePictureDialog.FileName) then
      PasAI_Raster_.SaveToJpegLS3File(SavePictureDialog.FileName)
  else if umlMultipleMatch('*.yv12', SavePictureDialog.FileName) then
      PasAI_Raster_.SaveToYV12File(SavePictureDialog.FileName)
  else
      SavePasAI_Raster(PasAI_Raster_, SavePictureDialog.FileName);
end;
                ...
                begin
                  ...
                  if not umlMultipleMatch(['-D3D', '-D2D', '-GPU', '-SOFT', '-GrayTheme'], n) then
                    if umlFileExists(n) then
                      begin
                        PasAI_Raster_ := NewPasAI_RasterFromFile(n);
                        if ViewIntf.Count = 0 then
                          begin
                            SetInputRaster(PasAI_Raster_);
                          end
                        else
                          begin
                            r.Offset(30, 30);
                            OpenSamplerEditor(r, PasAI_Raster_);
                          end;
                        disposeObject(PasAI_Raster_);
                        done_ := True;
                      end;
                end;
            end;
...
procedure TSampleEditorForm.SetInputRaster(r: TMPasAI_Raster);
begin
  Input.Assign(r);
  ViewIntf.Clear;
