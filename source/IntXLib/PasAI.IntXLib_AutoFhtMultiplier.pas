...
unit PasAI.IntXLib_AutoFhtMultiplier;
...
type
  ...
  public
    ...
    function Multiply(digitsPtr1: PCardinal; length1: UInt32; digitsPtr2: PCardinal; length2: UInt32; digitsResPtr: PCardinal): UInt32; override;
  end;
...
function TAutoFhtMultiplier.Multiply(digitsPtr1: PCardinal; length1: UInt32; digitsPtr2: PCardinal; length2: UInt32; digitsResPtr: PCardinal): UInt32;
var
...
begin
  ...
  if ((length1 < TConstants.AutoFhtLengthLowerBound) or
    ...
    begin
      result := F_classicMultiplier.Multiply(digitsPtr1, length1, digitsPtr2, length2, digitsResPtr);
      Exit;
    end;
  ...
  if ((digitsPtr1 = digitsPtr2) or (TDigitOpHelper.Cmp(digitsPtr1, length1, digitsPtr2, length2) = 0)) then
    begin
  ...
  TFhtHelper.ConvertDoubleToDigits(slice1, UInt32(Length(data1)), newLength, digitsResPtr);

  // Maybe check for validity using classic multiplication
  if (TIntX.GlobalSettings.ApplyFhtValidityCheck) then
    begin
      lowerDigitCount := Min(length2, Min(length1, TConstants.FhtValidityCheckDigitCount));
      ...
      F_classicMultiplier.Multiply(digitsPtr1, lowerDigitCount, digitsPtr2, lowerDigitCount, validationResultPtr);
      if (TDigitOpHelper.Cmp(validationResultPtr, lowerDigitCount, digitsResPtr, lowerDigitCount) <> 0) then
        begin
          raise EFhtMultiplicationException.Create(Format(PasAI.IntXLib_Strings.FhtMultiplicationError, [length1, length2], TIntX._FS));
        end;
    end;
