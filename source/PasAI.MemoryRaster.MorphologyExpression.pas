...
{ ****************************************************************************** }
unit PasAI.MemoryRaster.MorphologyExpression;
...
{$ENDIF FPC}
  TypInfo, Variants,
  PasAI.PascalStrings, PasAI.UPascalStrings, PasAI.Core, PasAI.UnicodeMixedLib, PasAI.ListEngine, PasAI.Status, PasAI.Parsing, PasAI.DFE, PasAI.MemoryStream,
  PasAI.Expression, PasAI.OpCode, PasAI.Geometry2D, PasAI.Geometry3D, PasAI.MemoryRaster, PasAI.DrawEngine;
...
type
  ...
  protected
    ...
    function MorphExp_DrawToPasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
    function MorphExp_LFBlendPasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
    function MorphExp_RFBlendPasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
    ...
    function MorphExp_PasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
    function MorphExp_CombineMorphToPasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
  ...
  public
    ...
    DebugViewer: TMPasAI_Raster; // debug used
  ...
  public
    ...
    // set input
    procedure InputPasAI_Raster(fileName: SystemString); overload;
    procedure InputPasAI_Raster(stream: TCore_Stream); overload;
    procedure InputPasAI_Raster(Raster: TMPasAI_Raster); overload;
    ...
    function Run(Raster: TMPasAI_Raster; script: U_String): Boolean; overload;
...
begin
  ...
  Exp.RegObjectOpM('Draw', 'Draw(token,TMorphologyPixel): draw raster ', {$IFDEF FPC}@{$ENDIF FPC}MorphExp_DrawToPasAI_Raster)^.Category := 'Morphology';
  Exp.RegObjectOpM('LFBlend', 'LFBlend(token..n): left to right blend raster ', {$IFDEF FPC}@{$ENDIF FPC}MorphExp_LFBlendPasAI_Raster)^.Category := 'Morphology';
  Exp.RegObjectOpM('RFBlend', 'RFBlend(token..n): right to left blend raster ', {$IFDEF FPC}@{$ENDIF FPC}MorphExp_RFBlendPasAI_Raster)^.Category := 'Morphology';
  ...
  Exp.RegObjectOpM('Raster', 'Raster(): build raster ', {$IFDEF FPC}@{$ENDIF FPC}MorphExp_PasAI_Raster)^.Category := 'Morphology';
  Exp.RegObjectOpM('CombineMorphToRaster', 'CombineMorphToRaster(TMorphologyPixel,Morphomatics data): Combine Morph as new raster ', {$IFDEF FPC}@{$ENDIF FPC}MorphExp_CombineMorphToPasAI_Raster)^.Category := 'Morphology';
...
function TExp_API.MorphExp_DrawToPasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
var
  meRT: TMorphExpRunTime;
  MorphPix_: TMorphologyPixel;
  drawToPasAI_RasterToken: U_String;
  step_: TMorphExpStep;
  i: Integer;
begin
  meRT := OpRunTime as TMorphExpRunTime;
  ...
  if (meRT.Step.InData.Raster <> nil) then
    begin
      ...
      drawToPasAI_RasterToken := '';
      case length(Param) of
        1: drawToPasAI_RasterToken := umlVarToStr(Param[0], False);
        2:
          begin
            drawToPasAI_RasterToken := umlVarToStr(Param[0], False);
            MorphPix_ := Param[1];
          end;
      end;
      step_ := FindStep(meRT, drawToPasAI_RasterToken);
      if step_ <> nil then
          meRT.Step.OutData.Assign(step_.OutData)
      else
        begin
          meRT.Step.OutData.Raster := NewPasAI_Raster();
...
function TExp_API.MorphExp_LFBlendPasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
var
  meRT: TMorphExpRunTime;
  i: Integer;
  n: U_String;
...
function TExp_API.MorphExp_RFBlendPasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
var
  meRT: TMorphExpRunTime;
  i: Integer;
  n: U_String;
...
function TExp_API.MorphExp_PasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
var
  meRT: TMorphExpRunTime;
begin
  meRT := OpRunTime as TMorphExpRunTime;
...
function TExp_API.MorphExp_CombineMorphToPasAI_Raster(OpRunTime: TOpCustomRunTime; var Param: TOpParam): Variant;
var
  meRT: TMorphExpRunTime;
  i: Integer;
  n: U_String;
...
var
  meRT: TMorphExpRunTime;
  SourcePasAI_RasterToken: U_String;
  step_: TMorphExpStep;
begin
  meRT := OpRunTime as TMorphExpRunTime;
  if (meRT.Step.InData = nil) or (meRT.Step.InData.FoundData = 0) then
  ...
  SourcePasAI_RasterToken := '';
  case length(Param) of
    1: SourcePasAI_RasterToken := umlVarToStr(Param[0], False);
  end;

  step_ := FindStep(meRT, SourcePasAI_RasterToken);
...
var
  rasterLst: TMemoryPasAI_RasterList;
  nR: TMPasAI_Raster;
  i: Integer;
  L, w, h: Integer;
  r: TRectV2;
  tmpFinal: TMPasAI_Raster;
begin
  DisposeObjectAndNil(DebugViewer);
  tmpFinal := nil;

  rasterLst := TMemoryPasAI_RasterList.Create;
  if Raster <> nil then
    begin
      nR := NewPasAI_Raster();
      nR.Assign(Raster);
...
procedure TMorphExp.InputPasAI_Raster(fileName: SystemString);
begin
  FInData.Clear;
  FInData.Raster := NewPasAI_RasterFromFile(fileName);
end;

procedure TMorphExp.InputPasAI_Raster(stream: TCore_Stream);
begin
  FInData.Clear;
  FInData.Raster := NewPasAI_RasterFromStream(stream);
end;

procedure TMorphExp.InputPasAI_Raster(Raster: TMPasAI_Raster);
begin
  FInData.Clear;
  FInData.Raster := NewPasAI_Raster();
...
function TMorphExp.Run(Raster: TMPasAI_Raster; script: U_String): Boolean;
var
  pl: TPascalStringList;
  i: Integer;
begin
  Reset;
  InputPasAI_Raster(Raster);
