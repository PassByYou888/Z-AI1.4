...
{$ENDIF FPC}
  PasAI.Core, PasAI.Cadencer, PasAI.Geometry2D, PasAI.Geometry.Low, PasAI.Geometry3D, PasAI.UnicodeMixedLib,
  PasAI.HashList.Templet,
  PasAI.ListEngine, PasAI.TextDataEngine, PasAI.MemoryRaster, PasAI.PascalStrings, PasAI.UPascalStrings,
  PasAI.Parsing, PasAI.Expression, PasAI.OpCode,
  PasAI.MemoryStream, PasAI.Notify, PasAI.BulletMovementEngine;
...
{$REGION 'Texture'}
  ...
  TDETexture_Pool_Decl = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<TDETexture>;

  TDETexture = class(TSequenceMemoryPasAI_Raster)
  private
...
{$REGION 'UI base'}
  ...
  TDrawEngine_UI_Pool_Decl = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<TDrawEngine_UIBase>;
...
{$REGION 'Bullet'}
  ...
  TBullet_Pool_Decl = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<TBullet_Base>;
...
{$REGION 'Sequence Animation'}
  TSequence_Animation_Base_Pool_Decl = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<TSequenceAnimationBase>;
...
{$REGION 'Particle'}
  ...
  TParticles_Pool_Decl = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<TParticles>;
  ...
  TParticle_Data_Pool = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<TParticle_Data>;
...
{$REGION 'DrawEngine pool'}
  ...
  TDrawEngine_Pool_Data_List_Decl = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<TDrawEngine_Pool_Data>;
...
{$REGION 'DrawEngine soft rasterization'}
  ...
  private
    ...
    function DEColor2RasterColor(const COLOR: TDEColor): TPasAI_RasterColor; overload;
    function DEColor2RasterColor(const COLOR: TDEColor; const Alpha: Byte): TPasAI_RasterColor; overload;
  public
  ...
  public
    ...
    procedure SetWorkMemory(m: TMPasAI_Raster);
...
{$REGION 'Text'}
  ...
  TScroll_Text_Data_Source_Pool_Decl = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<TScroll_Text_Data_Source>;
...
{$REGION 'Core'}
  ...
  public
    ...
    procedure SetSize(raster: TMPasAI_Raster); overload;
    ...
    property PasAI_Raster_: TDrawEngine_Raster read FRasterization;
    { draw interface }
  ...
  TRasterHelper_ = class helper for TMPasAI_Raster
  public
...
function DEColor(const C: TPasAI_RasterColor): TDEColor; overload;
function DEColor(const C: TPasAI_RasterColor; const Alpha: TDEFloat): TDEColor; overload;
function DEColor2RasterColor(const C: TDEColor): TPasAI_RasterColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}

function DColor2RColor(const C: TDEColor): TPasAI_RasterColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
function RColor2DColor(const C: TPasAI_RasterColor): TDEColor; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
...
uses Math, PasAI.DFE, PasAI.Status;
...
function DEColor(const C: TPasAI_RasterColor): TDEColor;
begin
  RColor2F(C, Result[0], Result[1], Result[2], Result[3]);
end;

function DEColor(const C: TPasAI_RasterColor; const Alpha: TDEFloat): TDEColor;
begin
  Result := DEColor(C);
  Result[3] := Alpha;
end;

function DEColor2RasterColor(const C: TDEColor): TPasAI_RasterColor;
begin
  Result := RColorF(C[0], C[1], C[2], C[3]);
end;

function DColor2RColor(const C: TDEColor): TPasAI_RasterColor;
begin
  Result := RColorF(C[0], C[1], C[2], C[3]);
end;

function RColor2DColor(const C: TPasAI_RasterColor): TDEColor;
begin
  RColor2F(C, Result[0], Result[1], Result[2], Result[3]);
end;
...
begin
  Result := PasAI.Geometry2D.IsEqual(Left, Dest.Left) and
    PasAI.Geometry2D.IsEqual(Top, Dest.Top) and
    PasAI.Geometry2D.IsEqual(Right, Dest.Right) and
    PasAI.Geometry2D.IsEqual(Bottom, Dest.Bottom) and
    PasAI.Geometry2D.IsEqual(Angle, Dest.Angle);
end;

function TDE4V.IsZero: Boolean;
begin
  Result :=
    PasAI.Geometry2D.IsZero(Left) and
    PasAI.Geometry2D.IsZero(Top) and
    PasAI.Geometry2D.IsZero(Right) and
    PasAI.Geometry2D.IsZero(Bottom);
end;
...
begin
  Result := PasAI.Geometry2D.TV2Rect4.Init(MakeRectV2, Angle).BoundRect;
end;

function TDE4V.Centroid: TDEVec;
begin
  Result := PasAI.Geometry2D.TV2Rect4.Init(MakeRectV2, Angle).Centroid;
end;
...
begin
  Result := PasAI.Geometry3D.Distance(MakeRectV2, Dest.MakeRectV2);
end;

function TDE4V.GetAngleDistance(Dest: TDE4V): TDEFloat;
begin
  Result := PasAI.Geometry3D.AngleDistance(Angle, Dest.Angle);
end;
...
var
  d1, d2: Double;
begin
  d1 := PasAI.Geometry3D.AngleRollDistanceDeltaTime(Angle, Dest.Angle, rSpeed);
  d2 := PasAI.Geometry3D.MovementDistanceDeltaTime(MakeRectV2, Dest.MakeRectV2, mSpeed);
  if d1 > d2 then
      Result := d1
  else
      Result := d2;
      ...
      if not FIsShadow then
        begin
          ...
          for i := (width * height) - 1 downto 0 do
              bits_^[i] := RColor(0, 0, 0, TPasAI_RasterColorEntry(Bits^[i]).a);
      ...
      if PrepareDraw and (Size > 5) then
        begin
          ...
          if (FStartDrawShadowIndex >= 0) then
            begin
              Data^.r := PasAI.Geometry2D.RectOffset(Data^.r, FScreenShadowOffset);
...
function TDrawEngine_Raster.DEColor2RasterColor(const COLOR: TDEColor): TPasAI_RasterColor;
begin
  Result := PasAI_RasterColorF(COLOR[0], COLOR[1], COLOR[2], COLOR[3]);
end;

function TDrawEngine_Raster.DEColor2RasterColor(const COLOR: TDEColor; const Alpha: Byte): TPasAI_RasterColor;
begin
  Result := PasAI_RasterColorF(COLOR[0], COLOR[1], COLOR[2], COLOR[3]);
end;
...
begin
  ...
  if t is TMPasAI_Raster then
      TMPasAI_Raster(t).ProjectionTo(FMemory, TV2Rect4.Init(Sour.MakeRectV2, Sour.Angle), TV2Rect4.Init(Dest.MakeRectV2, Dest.Angle), True, Alpha)
  else
...
procedure TDrawEngine_Raster.SetWorkMemory(m: TMPasAI_Raster);
begin
...
begin
  ...
  FDefaultTexture.SetSize(2, 2, PasAI_RasterColorF(0, 0, 0, 1));
...
procedure TDrawEngine.SetSize(raster: TMPasAI_Raster);
begin
...
begin
  ...
  for i := 0 to rp.Count - 1 do
    begin
      t := rp[i]^.Data2 as TMPasAI_Raster;
...
begin
  ...
  if t is TSequenceMemoryPasAI_Raster then
    begin
      Result.width := TSequenceMemoryPasAI_Raster(t).width;
      Result.height := TSequenceMemoryPasAI_Raster(t).height;
      Result.Total := TSequenceMemoryPasAI_Raster(t).Total;
      Result.Column := TSequenceMemoryPasAI_Raster(t).Column;
    end;
    ...
    for i := Low(FPictureFlushInfo) to High(FPictureFlushInfo) do
      begin
        ...
        if not rl.Data2Exists(ptex^.Source) then
          begin
            if ptex^.Source is TMPasAI_Raster then
...
Hooked_OnCheckThreadSynchronize := PasAI.Core.OnCheckThreadSynchronize;
PasAI.Core.OnCheckThreadSynchronize := {$IFDEF FPC}@{$ENDIF FPC}DoCheckThreadSynchronize;
Draw_Engine_Auto_Hook_Check_Thread := False;

finalization

PasAI.Core.OnCheckThreadSynchronize := Hooked_OnCheckThreadSynchronize;
