...
{ * Audio Fingerprint                                                          * }
{ ****************************************************************************** }
unit PasAI.Learn.AudioPrint;

{$I PasAI.Define.inc}

interface

uses PasAI.Core, PasAI.PascalStrings, PasAI.UPascalStrings, PasAI.Status, PasAI.UnicodeMixedLib, PasAI.Learn.Type_LIB, PasAI.Learn;
...
function FreqToOctave(freq, base: TLFloat): TLFloat;
begin
  Result := log10(freq / base) / log10(2.0); // logarithmus dualis
end;

function IndexToFreq(i, frame_size, Sample_Rate: TLInt): TLFloat;
begin
  Result := i * Sample_Rate / frame_size;
end;

function FreqToIndex(freq: TLFloat; frame_size, Sample_Rate: TLInt): TLInt;
begin
  Result := round(frame_size * freq / Sample_Rate);
end;

procedure NormalizeVector(var vector: TLVec; norm: TLFloat; Threshold: TLFloat);
var
  i: TLInt;
begin
...
function EuclideanNorm(vector: TLVec): TLFloat;
var
...
procedure PrepareHammingWindow(var Data: TLVec);
var
  i, n: TLInt;
  scale: TLFloat;
begin
...
procedure ApplyWindow(var Data: TLVec; window: TLVec; Size: TLInt; scale: TLFloat);
var
  i: TLInt;
begin
  i := 0;
...
type
  TAudioConsumer = class(TCore_Object)
  public
  ...
  TBitStringWriter = class(TCore_Object)
  public
  ...
  TBitStringReader = class(TCore_Object)
  public
    FValue: TPascalString;
    FValueIter: TLInt;
    FBuffer: TUInt32;
  ...
  TIntegralImage = class(TCore_Object)
  public
    FImage: TCPImage;
    procedure Transform;
  ...
  TClassifier = class(TCore_Object)
  public
    FFilter: TFilter;
  ...
  TCombinedBuffer = class(TCore_Object)
  public
    FOffset: TLInt;
  ...
  TFFTFrameConsumer = class(TCore_Object)
  public
    procedure Consume(frame: TFFTFrame); virtual; abstract;
  end;

  TFFTLib = class(TCore_Object)
  public
    FWindow: TLVec;
    FFrameSize: TLInt;
    FFrameSizeH: TLInt;
  ...
  TFeatureVectorConsumer = class(TCore_Object)
  public
    constructor Create;
    destructor Destroy; override;
  ...
  TFingerprintCalculator = class(TCore_Object)
  public
  ...
  TFingerprintCompressor = class(TCore_Object)
  public
  ...
  TFingerprintDecompressor = class(TCore_Object)
  public
    FResult: TU32Vec;
    FBits: TBytes_;
  ...
  TFingerprinterConfiguration = class(TCore_Object)
  public
  ...
  TMovingAverage = class(TCore_Object)
  public
    FBuffer: TI16Vec;
    FSize: TLInt;
    FOffset: TLInt;
...
function BuildFilter(var Filter: TI16Vec; factor: TLFloat; tap_count: TLInt; phase_count, scale, _Type: TLInt): boolean;

{ 0th order modified bessel function of the first kind. }

  function bessel(x: TLFloat): TLFloat;
  var
  ...
  function Clip16(a: TLInt; amin, amax: TInt16): TInt16;
  begin
    if (a < amin) then
        Result := amin
    else if (a > amax) then
...
begin
  ...
  if (lLength > kMaxBufferSize) then
    begin
      DoStatus('Z.Learn.AudioPrint::AudioProcessor::Resample() -- Resampling overwrote output buffer.');
  ...
  if (remaining > 0) then
    ...
    begin
      DoStatus('Z.Learn.AudioPrint::AudioProcessor::Resample() -- Resampling overread input buffer.');
      remaining := 0;
    end;
...
begin
  if (NumChannels <= 0) then
    begin
      DoStatus('Z.Learn.AudioPrint::AudioProcessor::Reset() -- No audio channels.');
      Result := False;
      Exit;
    end;
  ...
  if (SampleRate <= kMinSampleRate) then
    begin
      DoStatus('Z.Learn.AudioPrint::AudioProcessor::Reset() -- Sample rate less than 0 1,      kMinSampleRate, sample_rate');
      Result := False;
      Exit;
    end;
    ...
    begin
      ...
      if (FBufferSize = FBufferOffset) then
        begin
          ...
          if (FBufferSize = FBufferOffset) then
            begin
              // DEBUG("Z.Learn.AudioPrint::AudioProcessor::Consume() -- Resampling failed?");
              Exit;
            end;
        end;
    end;
