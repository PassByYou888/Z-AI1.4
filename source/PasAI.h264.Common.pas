...
uses
  PasAI.h264.Types, PasAI.h264.Stats, PasAI.FPC.GenericList, PasAI.Core, PasAI.MemoryRaster, Math;
...
procedure YV12ToPasAI_Raster(const luma_ptr, u_ptr, v_ptr: uint8_p; const w, h, stride, stride_cr: int32_t; const dest: TMPasAI_Raster; const forceITU_BT_709, lumaFull: Boolean); overload;
procedure PasAI_RasterToYV12(const sour: TMPasAI_Raster; const luma_ptr, u_ptr, v_ptr: uint8_p; const w, h: int32_t); overload;
...
procedure YV12ToPasAI_Raster(const luma_ptr, u_ptr, v_ptr: uint8_p; const w, h, stride, stride_cr: int32_t; const dest: TMPasAI_Raster; const forceITU_BT_709, lumaFull: Boolean);
...
var
  ...
  row1, row2: PPasAI_RasterColorEntry;
...
begin
  ...
  for y := 0 to dest.height shr 1 - 1 do
    begin
      row1 := PPasAI_RasterColorEntry(dest.ScanLine[y * 2]);
      row2 := PPasAI_RasterColorEntry(dest.ScanLine[y * 2 + 1]);
...
procedure PasAI_RasterToYV12(const sour: TMPasAI_Raster; const luma_ptr, u_ptr, v_ptr: uint8_p; const w, h: int32_t);
  function Clip(c: int32_t): uint8_t;
  begin
    Result := uint8_t(c);
    if c > 255 then
