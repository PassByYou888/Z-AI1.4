...
{$ENDIF FPC}
  PasAI.Core, Types, PasAI.MemoryStream, PasAI.PascalStrings, PasAI.UPascalStrings;
...
procedure Clip(const sour_, background_: TRectV2; var output: TRectV2); {$IFDEF INLINE_ASM} inline; {$ENDIF INLINE_ASM} overload;
function Clip(const sour, background: TRectV2): TRectV2; {$IFDEF INLINE_ASM} inline; {$ENDIF INLINE_ASM} overload;
...
{$REGION 'Nearest_Box_Tool'}
  ...
  TNearest_Box_Tool_ = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<TNearest_Box_Data>;

  TNearest_Box_List_ = {$IFDEF FPC}specialize {$ENDIF FPC} TPasAI_Raster_BL<PNearest_Box_Data>;
...
uses PasAI.UnicodeMixedLib, PasAI.Geometry3D, PasAI.Geometry.Low, PasAI.DFE, PasAI.Status;
...
{$I PasAI.Geometry.Split.Header.inc}
{$I PasAI.Geometry.Split.Body.inc}
{$IFDEF FPC}
...
begin
  Result :=
    PasAI.Geometry2D.IsZero(LeftTop) and
    PasAI.Geometry2D.IsZero(RightTop) and
    PasAI.Geometry2D.IsZero(RightBottom) and
    PasAI.Geometry2D.IsZero(LeftBottom);
end;
...
begin
  Result := PasAI.Geometry2D.BoundRect(LeftTop, RightTop, RightBottom, LeftBottom);
end;
...
begin
  if length(buff) <> 4 then
    begin
      Result := TV2Rect4.Init(PasAI.Geometry2D.BoundRect(buff));
    end
  else
    with Result do
      begin
