...
unit PasAI.MemoryRaster.JPEG.BitStream;

{$I PasAI.Define.inc}

interface

uses PasAI.Core, PasAI.PascalStrings, PasAI.UPascalStrings, PasAI.MemoryStream, PasAI.UnicodeMixedLib, PasAI.Status, PasAI.MemoryRaster.JPEG.Type_LIB;
...
type
  ...
  public
    ...
    ThisByte: PByte;
    NextByte: PByte;
    constructor Create(S: TMS64); virtual;
  ...
  public
    constructor Create(S: TMS64); override;
...
constructor TBitReader.Create(S: TMS64);
begin
  inherited Create;
...
begin
  Result := NativeUInt(FNextInMem) - NativeUInt(TMS64(FStream).Memory);
end;

procedure TMemoryBitReader.SetStreamPos(const Value: Int64);
begin
  FNextInMem := TMS64(FStream).Memory;
  inc(FNextInMem, Value);
end;

constructor TMemoryBitReader.Create(S: TMS64);
begin
  inherited Create(S);
  { Set pointers in memory }
  FNextInMem := TMS64(S).Memory;
...
begin
  ...
  FStream.Position := NativeUInt(FNextInMem) - NativeUInt(TMS64(FStream).Memory);
end;
...
constructor TBitWriter.Create(S: TMS64);
begin
  inherited Create;
  FStream := S;
end;
