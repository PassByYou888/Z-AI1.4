unit PasAI.diocp_ex_http_common;
...
uses
  PasAI.utils_strings, SysUtils, PasAI.utils_dvalue
...
{$IF CompilerVersion < 18}
  TBytes = PasAI.utils_strings.TBytes;
{$IFEND}
  PRange = ^TRangeRec;
...
{$ELSE}
{$IFDEF USE_ZLIBExGZ}
    PasAI.ZLibEx.ZCompress(pvBuilder.Memory, pvBuilder.length, lvOutBuf, lvOutBytes);
{$ELSE}
...
var
  ...
  l: Integer;
begin
  l := PasAI.utils_lzo.lzo_compressdestlen(pvBuilder.length);
  SetLength(lvOutBytes, l);
  l := PasAI.utils_lzo.lzo_compress(PAnsiChar(pvBuilder.Memory), pvBuilder.length,
...
var
  ...
  l: Integer;
begin
  l := PasAI.utils_lzo.lzo_decompressdestlen(PAnsiChar(pvBuilder.Memory));
  SetLength(lvOutBytes, l);
  l := PasAI.utils_lzo.lzo_decompress(PAnsiChar(pvBuilder.Memory), pvBuilder.length,
    PAnsiChar(@lvOutBytes[0]));
