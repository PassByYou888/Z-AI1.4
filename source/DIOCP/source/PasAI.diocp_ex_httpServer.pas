(*
  ...
  *   加入WebSocket支持(感谢音儿)
*)
unit PasAI.diocp_ex_httpServer;
...
uses
  Classes, StrUtils, SysUtils, PasAI.SHA, PasAI.utils_strings 

  {$IFDEF QDAC_QWorker}, qworker{$ENDIF}
  {$IFDEF DIOCP_Task}, PasAI.diocp_task{$ENDIF}
  , PasAI.diocp_tcp_server, PasAI.utils_queues, PasAI.utils_hashs, PasAI.utils_dvalue
  , PasAI.diocp_ex_http_common
  , PasAI.diocp_res
  , PasAI.utils_objectPool, PasAI.utils_safeLogger, Windows, PasAI.utils_threadinfo, SyncObjs,
  PasAI.utils_BufferPool,  PasAI.utils_websocket,  PasAI.utils_base64, DateUtils;
...
type
  ...
  TDiocpHttpCookie = PasAI.diocp_ex_http_common.TDHttpCookie;
...
begin
  ...
  begin
    ...
    raise Exception.CreateFmt('(%d,%d)当前对象已经被其他线程正在使用::%s',
       [PasAI.utils_strings.GetCurrentThreadID, FThreadID, s]);
  end;
  FThreadID := PasAI.utils_strings.GetCurrentThreadID;
end;
...
var
  ...
  s:string;
begin
  lvThreadID := PasAI.utils_strings.GetCurrentThreadID;
