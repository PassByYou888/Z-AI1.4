{ ****************************************************************************** }
{ * fast Histogram of Oriented Gradient                                        * }
{ ****************************************************************************** }

unit PasAI.MemoryRaster.Histogram;

{$I PasAI.Define.inc}

interface

uses PasAI.Core, PasAI.MemoryRaster, PasAI.Geometry2D, PasAI.Learn.Type_LIB;

type
  THOGTable = class(TCore_Object)
  private type
    TItpRec = record
  ...
  private
    ...
    procedure ComputeRGB_Diff(img: TMPasAI_Raster; const M: PLMatrix);
  ...
  public
    ori: THRecArray;
    OriDim: TLInt;
    constructor Create(Table: THOGTable; img: TMPasAI_Raster);
    constructor CreateAntiLight(Table: THOGTable; img: TMPasAI_Raster);
    destructor Destroy; override;
    ...
    procedure BuildViewer(output: TMPasAI_Raster);
  end;
...
uses Math, PasAI.Status, PasAI.Learn;
...
constructor THOGTable.Create(const numOriHalf, numOriFull, cellSize: TLInt);
  function arcTanXY(const x, y: TLFloat): TLInt;
  begin
...
procedure THOG.ComputeRGB_Diff(img: TMPasAI_Raster; const M: PLMatrix);
{$IFDEF Parallel}
{$IFDEF FPC}
  procedure Nested_ParallelFor(pass: Integer);
  var
    x, xDiffs, yDiffs, magni: TLInt;
    pcl, pcr, pct, pcb: TPasAI_RasterColorEntry;
  begin
  ...
  begin
    for cellX := 1 to nCX do
      begin
        ...
        NW[0] := PasAI.Learn.AP_Sqr(HP^.normH + ori[cellY - 1, cellX].normH + ori[cellY, cellX - 1].normH + ori[cellY - 1, cellX - 1].normH);
        NW[1] := PasAI.Learn.AP_Sqr(HP^.normH + ori[cellY + 1, cellX].normH + ori[cellY, cellX - 1].normH + ori[cellY + 1, cellX - 1].normH);
        NW[2] := PasAI.Learn.AP_Sqr(HP^.normH + ori[cellY + 1, cellX].normH + ori[cellY, cellX + 1].normH + ori[cellY + 1, cellX + 1].normH);
        NW[3] := PasAI.Learn.AP_Sqr(HP^.normH + ori[cellY - 1, cellX].normH + ori[cellY, cellX + 1].normH + ori[cellY - 1, cellX + 1].normH);
        ...
        if AntiLight then
          begin
            ...
            for n := 0 to length(HP^.feat) - 1 do
                LAdd(Sum, HP^.feat[n]);
            for n := 0 to length(HP^.feat) - 1 do
                HP^.feat[n] := PasAI.Learn.AP_Sqr(LSafeDivF(HP^.feat[n], Sum));
          end;
      end;
  end;
{$ENDIF FPC}
...
constructor THOG.Create(Table: THOGTable; img: TMPasAI_Raster);
begin
  inherited Create;
  initialHist(
...
constructor THOG.CreateAntiLight(Table: THOGTable; img: TMPasAI_Raster);
begin
  inherited Create;
  initialHist(
    img.width - img.width mod Table.cSiz,
...
procedure THOG.BuildViewer(output: TMPasAI_Raster);
var
  bp, EP, cp, p: TVec2;
  numOri, MaxIdx: TLInt;
  MaxValue: TLInt;
            ...
            begin
              ...
              if pixValue > MaxValue then
                ...
                begin
                  ...
                  output.LineF(PointLerpTo(p, cp, cSiz), p, PasAI_RasterColor(pixValue, pixValue, pixValue, pixValue), True);
                end;
            end;
        end;
    end;
...
var
  ...
  M: TLMatrix;
begin
  img := NewPasAI_RasterFromFile(inputfile);
