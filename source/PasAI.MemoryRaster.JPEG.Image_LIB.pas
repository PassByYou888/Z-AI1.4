...
unit PasAI.MemoryRaster.JPEG.Image_LIB;
...
type
  ...
  // TJpegImage is a non-VCL component that can be used to load and save Jpeg files.
  // It is best to create just one instance of TJpegImage, and use it over and over again to load Jpeg files,
  // because this way memory for the coefficients and bitmaps will be reused,
  // instead of repeatedly allocated/deallocated (which would happen if you each time create a new TJpegImage instance).
  ...
  public
    ...
    // Load a Jpeg image from the stream. It is best to use a TMS64
    // because the bitreader (which causes most reads to the stream) has
    // a specially optimized version to read from TMS64 streams. The
...
procedure TJpegImage.EntropyDecodeSkip(S: TMS64);
...
function TJpegImage.LoadMarker(S: TMS64): byte;
var
...
begin
  ...
  FCoderStream := TMS64.CustomCreate(512 * 1024);
...
procedure TJpegImage.LoadFromStream(Stream: TMS64);
...
var
  FS: TMS64;
begin
  FS := TMS64.CustomCreate(512 * 1024);
  try
    SaveToStream(FS);
...
procedure TJpegImage.SaveToStream(Stream: TMS64);
const
  cFF: byte = $FF;
var
  i: integer;
