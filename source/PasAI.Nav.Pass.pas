...
uses SysUtils, PasAI.Core, Math, PasAI.Geometry2D, PasAI.Nav.Polygon, PasAI.MovementEngine;
...
type
  ...
  TPassState = record
    ...
    function Enabled(ignore: TCore_ListForObj): Boolean; overload;
  end;

  PPassState = ^TPassState;

  TBasePass = class(TCore_Object)
  private
    FDataList: TCore_List;
  ...
  public
    constructor Create(Owner_: TPolyPassManager);
    ...
    procedure BuildPass(ExpandDist_: TGeoFloat); virtual; abstract;
  ...
  private
    FPoint: TVec2;
  public
    constructor Create(Owner_: TPolyPassManager; APoint: TVec2);
    procedure BuildPass(ExpandDist_: TGeoFloat); override;
  ...
  private
    ...
    FCachePoint: TVec2;
  public
    constructor Create(Owner_: TPolyPassManager; Poly_: TPolyManagerChildren; APolyIndex: Integer; ExpandDist_: TGeoFloat);
    procedure BuildPass(ExpandDist_: TGeoFloat); override;
    function pass(Pass_: TBasePass): Boolean; override;
    function GetPosition: TVec2; override;
  end;
  ...
  public
    constructor Create(Owner_: TPolyPassManager;
      ANavBio: TNavBio; ANavBioIndex: Integer; ExpandDist_: TGeoFloat; AFastEdgePass: Boolean);
    destructor Destroy; override;
    procedure BuildPass(ExpandDist_: TGeoFloat); override;
    function pass(Pass_: TBasePass): Boolean; override;
    function GetPosition: TVec2; override;
  ...
  TPolyPassManager = class(TCore_Object)
  private type
  ...
  public
    constructor Create(PolyManager_: TPolyManager; NavBioManager_: TNavBioManager; ExpandDistance_: TGeoFloat);
    destructor Destroy; override;

    function PointOk(ExpandDist_: TGeoFloat; pt: TVec2): Boolean; overload;
    // include dynamic poly,ignore object
    function PointOk(ExpandDist_: TGeoFloat; pt: TVec2; ignore: TCore_ListForObj): Boolean; overload;

    function LineIntersect(ExpandDist_: TGeoFloat; lb, le: TVec2): Boolean; overload;
    // include dynamic poly,ignore object
    function LineIntersect(ExpandDist_: TGeoFloat; lb, le: TVec2; ignore: TCore_ListForObj): Boolean; overload;

    function PointOnCache(ExpandDist_: TGeoFloat; poly1: TPolyManagerChildren; idx1: Integer): Boolean;
    function LineIntersectCache(ExpandDist_: TGeoFloat;
    ...
    function Add(Conn_: TBasePass; const NowBuildPass: Boolean): Integer;
    procedure Delete(idx: Integer); overload;
    procedure Delete(Conn_: TBasePass); overload;
    procedure Clear;
    function Count: Integer;
  ...
  TNavBio = class(TCore_InterfacedObject, IMovementEngineInterface)
  private
    FOwner: TNavBioManager;
    FPassManager: TPolyPassManager;
    // in Pass manager
    FEdgePassList: TCore_ListForObj;
    FPosition: TVec2;
  ...
  public
    ...
    property CollisionList: TCore_ListForObj read FCollisionList;
    ...
    property EdgePassList: TCore_ListForObj read FEdgePassList;
  end;

  TNavBioNotify = procedure(Sender: TNavBio) of object;

  TNavBioManager = class(TCore_Persistent)
  private
    FNavBioList: TCore_ListForObj;
...
uses PasAI.Nav.PathFinding, PasAI.Geometry3D;
...
function TPassState.Enabled(ignore: TCore_ListForObj): Boolean;
begin
...
constructor TBasePass.Create(Owner_: TPolyPassManager);
begin
  inherited Create;
  FDataList := TCore_List.Create;
  FOwner := Owner_;
  FPassIndex := -1;
end;
...
constructor TPointPass.Create(Owner_: TPolyPassManager; APoint: TVec2);
begin
  inherited Create(Owner_);
  FPoint := APoint;
end;

procedure TPointPass.BuildPass(ExpandDist_: TGeoFloat);
var
  i: Integer;
  b: TBasePass;
  pt: TVec2;
begin
  ...
  for i := 0 to FOwner.Count - 1 do
    begin
      b := FOwner.pass[i];
      if (b <> Self) and (pass(b)) and (not Exists(b)) and (not FOwner.FPolyManager.LineIntersect(ExpandDist_, pt, b.GetPosition)) then
          Add(b);
    end;
end;
...
constructor TPolyPass.Create(Owner_: TPolyPassManager; Poly_: TPolyManagerChildren; APolyIndex: Integer; ExpandDist_: TGeoFloat);
begin
  inherited Create(Owner_);
  FPoly := Poly_;
  FPolyIndex := APolyIndex;
  FExtandDistance := ExpandDist_;
  FCachePoint := FPoly.Expands[FPolyIndex, FExtandDistance];
end;

procedure TPolyPass.BuildPass(ExpandDist_: TGeoFloat);
var
  pt: TVec2;
  i: Integer;
  b: TBasePass;
  ...
  for i := 0 to FOwner.Count - 1 do
    begin
      ...
      if b is TPolyPass then
        begin
          p := TPolyPass(b);
          if (p <> Self) and (pass(p)) and (not Exists(p)) and (not FOwner.LineIntersectCache(ExpandDist_, FPoly, FPolyIndex, p.FPoly, p.FPolyIndex)) then
              Add(p);
        end
      else
        begin
          if (b <> Self) and (pass(b)) and (not Exists(b)) and (not FOwner.LineIntersect(ExpandDist_, pt, b.GetPosition)) then
              Add(b);
        end;
    end;
end;
...
constructor TNavigationBioPass.Create(Owner_: TPolyPassManager;
  ANavBio: TNavBio; ANavBioIndex: Integer; ExpandDist_: TGeoFloat; AFastEdgePass: Boolean);
begin
  inherited Create(Owner_);
  FNavBio := ANavBio;
...
procedure TNavigationBioPass.BuildPass(ExpandDist_: TGeoFloat);
var
  pt: TVec2;
  i: Integer;
  b: TBasePass;
      ...
      for i := 0 to FOwner.Count - 1 do
        begin
          ...
          if (b = Self) and (pass(b)) and (not Exists(b)) and
            (not FOwner.FNavBioManager.LineIntersect(ExpandDist_ - 1, pt, b.GetPosition)) then
              Add(b);
        end;
    end
  else
    begin
      ...
      for i := 0 to FOwner.Count - 1 do
        begin
          ...
          if (b <> Self) and (pass(b)) and (not Exists(b)) and
            (not FOwner.LineIntersect(ExpandDist_, pt, b.GetPosition)) and
            (not FOwner.FNavBioManager.LineIntersect(ExpandDist_ - 1, pt, b.GetPosition)) then
              Add(b);
        end;
    end;
...
constructor TPolyPassManager.Create(PolyManager_: TPolyManager; NavBioManager_: TNavBioManager; ExpandDistance_: TGeoFloat);
begin
  inherited Create;
  FPolyManager := PolyManager_;
  FNavBioManager := NavBioManager_;
  FPassList := TCore_ListForObj.Create;
  FExtandDistance := ExpandDistance_;
...
function TPolyPassManager.PointOk(ExpandDist_: TGeoFloat; pt: TVec2): Boolean;
begin
  Result := FPolyManager.PointOk(ExpandDist_, pt);
end;

function TPolyPassManager.PointOk(ExpandDist_: TGeoFloat; pt: TVec2; ignore: TCore_ListForObj): Boolean;
begin
  Result := PointOk(ExpandDist_, pt);
  if Result then
      Result := FNavBioManager.PointOk(ExpandDist_, pt, ignore);
end;

function TPolyPassManager.LineIntersect(ExpandDist_: TGeoFloat; lb, le: TVec2): Boolean;
begin
  Result := FPolyManager.LineIntersect(ExpandDist_, lb, le);
end;

function TPolyPassManager.LineIntersect(ExpandDist_: TGeoFloat; lb, le: TVec2; ignore: TCore_ListForObj): Boolean;
begin
  Result := LineIntersect(ExpandDist_, lb, le);
  if not Result then
      Result := FNavBioManager.LineIntersect(ExpandDist_, lb, le, ignore);
end;

function TPolyPassManager.PointOnCache(ExpandDist_: TGeoFloat; poly1: TPolyManagerChildren; idx1: Integer): Boolean;
begin
  if FPointInCache[poly1.index][idx1] = csNoCompute then
    begin
      Result := PointOk(ExpandDist_ - 1, poly1.Expands[idx1, ExpandDist_ + 1]);
...
function TPolyPassManager.LineIntersectCache(ExpandDist_: TGeoFloat;
  poly1: TPolyManagerChildren; idx1: Integer; poly2: TPolyManagerChildren; idx2: Integer): Boolean;
begin
  if FIntersectCache[poly1.index][idx1][poly2.index][idx2] = csNoCompute then
    begin
      Result := LineIntersect(ExpandDist_, poly1.Expands[idx1, ExpandDist_ + 1], poly2.Expands[idx2, ExpandDist_ + 1]);
...
var
  i: Integer;
  d: TNavigationBioPass;
  List: TCore_ListForObj;
begin
  DeleteNavBio(NavBio);

  List := TCore_ListForObj.Create;
...
procedure TPolyPassManager.BuildPass;
  ...
  begin
    for i := 0 to Poly_.Count - 1 do
      if PointOnCache(FExtandDistance, Poly_, i) then
        begin
...
function TPolyPassManager.Add(Conn_: TBasePass; const NowBuildPass: Boolean): Integer;
begin
  Assert(Conn_.FOwner = Self);
  if (Conn_ is TPointPass) then
    begin
      if (PointOk(FExtandDistance - 1, Conn_.GetPosition)) and (NowBuildPass) then
          Conn_.BuildPass(FExtandDistance - 1);
    end
  else if (Conn_ is TPolyPass) then
    begin
      if (PointOnCache(FExtandDistance, TPolyPass(Conn_).FPoly, TPolyPass(Conn_).FPolyIndex)) and (NowBuildPass) then
          Conn_.BuildPass(FExtandDistance - 1);
    end;
  Result := FPassList.Add(Conn_);
end;
...
procedure TPolyPassManager.Delete(Conn_: TBasePass);
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
    if FPassList[i] = Conn_ then
      begin
        Delete(i);
        Break;
      end;
...
procedure TNavBioProcessState.Init(Owner_: TNavBio);
begin
  Owner := Owner_;
...
constructor TNavBio.Create(Owner_: TNavBioManager; PassManager_: TPolyPassManager);
begin
  inherited Create;
  FOwner := Owner_;
  FPassManager := PassManager_;
  FEdgePassList := TCore_ListForObj.Create;
  ...
  FCollisionList := TCore_ListForObj.Create;
...
begin
  inherited Create;
  FNavBioList := TCore_ListForObj.Create;
  FBounceWaitTime := 5.0;
  FSlices := 5;
  FSmooth := True;
...
function TNavBioManager.PointOk(ExpandDist_: TGeoFloat; pt: TVec2; ignore: TCore_ListForObj): Boolean;
...
begin
  for i := 0 to Count - 1 do
    if not IsIgnore(Items[i]) then
      with Items[i] do
        if (IsStatic) and (Detect_Circle2Circle(pt, DirectPosition, ExpandDist_, radius)) then
          begin
            Result := False;
            Exit;
          end;
...
function TNavBioManager.PointOk(ExpandDist_: TGeoFloat; pt: TVec2): Boolean;
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
...
function TNavBioManager.LineIntersect(ExpandDist_: TGeoFloat; lb, le: TVec2; ignore: TCore_ListForObj): Boolean;
  function IsIgnore(v: TNavBio): Boolean;
  begin
    Result := (ignore <> nil) and (ignore.IndexOf(v) >= 0);
  end;
...
begin
  for i := 0 to Count - 1 do
    if not IsIgnore(Items[i]) then
      with Items[i] do
        if (IsStatic) and (Detect_Circle2Line(DirectPosition, radius + ExpandDist_, lb, le)) then
          begin
            Result := True;
            Exit;
          end;
...
function TNavBioManager.LineIntersect(ExpandDist_: TGeoFloat; lb, le: TVec2): Boolean;
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
