...
03/06/2007 V1.06 TIcsWndHandler got a new public event OnMessage (suggested by
                 Primo?Gabrijelcic <primoz@gabrijelcic.org>). D7 code explorer
...
unit PasAI.OverbyteIcsWndControl;
{$ENDIF}
...
{$ELSE MSWINDOWS}
    ...
    FHandle := PasAI.Ics.Posix.Messages.AllocateHWND(WndProc);
    AllocateMsgHandlers;
{$ENDIF}
end;
...
{$ELSE MSWINDOWS}
    FreeMsgHandlers;
    PasAI.Ics.Posix.Messages.DestroyWindow(FHandle);
    FHandle := 0;
        ...
        with MsgRec do begin
          ...
          {$ELSE} //Posix
                ...
                if { (not IsBadReadPtr(Obj, GUIDOffSet + SizeOf(INT_PTR))) and }
                   (PUINT_PTR(WParam + PasAI.Ics.Posix.Messages.WPARAM(GUIDOffSet))^ = WParam) and
                ...
                Obj := TObject(WParam);
                if {(not IsBadReadPtr(Obj, GUIDOffSet + SizeOf(INT_PTR))) and }
                   (PINT_PTR(WParam + PasAI.Ics.Posix.Messages.WPARAM(GUIDOffSet))^ = LParam) and
