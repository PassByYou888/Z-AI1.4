...
uses Types, PasAI.Core,
  PasAI.Geometry.Low, PasAI.Geometry2D, PasAI.PascalStrings, PasAI.UPascalStrings, PasAI.UnicodeMixedLib;
...
begin
  Result.Buff := PasAI.Geometry.Low.MatrixMultiply(Lhs.Buff, Rhs.Buff);
end;
...
begin
  Result.Buff := Buff;
  det := PasAI.Geometry.Low.MatrixDeterminant(Result.Buff);
  if Abs(det) < Epsilon then
      Result.Buff := PasAI.Geometry.Low.IdentityHmgMatrix
  else
    begin
      PasAI.Geometry.Low.AdjointMatrix(Result.Buff);
      PasAI.Geometry.Low.ScaleMatrix(Result.Buff, 1 / det);
    end;
end;
...
begin
  Result.Buff := Buff;
  PasAI.Geometry.Low.TranslateMatrix(Result.Buff, v);
end;

function TMatrix4.Normalize: TMatrix4;
begin
  Result.Buff := Buff;
  PasAI.Geometry.Low.NormalizeMatrix(Result.Buff);
end;

function TMatrix4.Transpose: TMatrix4;
begin
  Result.Buff := Buff;
  PasAI.Geometry.Low.TransposeMatrix(Result.Buff);
end;

function TMatrix4.AnglePreservingInvert: TMatrix4;
begin
  Result.Buff := PasAI.Geometry.Low.AnglePreservingMatrixInvert(Buff);
end;

function TMatrix4.Determinant: TGeoFloat;
begin
  Result := PasAI.Geometry.Low.MatrixDeterminant(Buff);
end;
...
begin
  Result.Buff := PasAI.Geometry.Low.Pitch(Buff, angle);
end;

function TMatrix4.Roll(angle: TGeoFloat): TMatrix4;
begin
  Result.Buff := PasAI.Geometry.Low.Roll(Buff, angle);
end;

function TMatrix4.Turn(angle: TGeoFloat): TMatrix4;
begin
  Result.Buff := PasAI.Geometry.Low.Turn(Buff, angle);
end;
...
begin
  Result.Buff := PasAI.Geometry.Low.VectorTransform(Lhs.Buff, Rhs.Buff);
end;
...
begin
  Result.Buff := PasAI.Geometry.Low.VectorTransform(Lhs.Buff, Rhs);
end;

class operator TVector4.Multiply(const Lhs: TMat4; const Rhs: TVector4): TVector4;
begin
  Result.Buff := PasAI.Geometry.Low.VectorTransform(Rhs.Buff, Lhs);
end;
...
begin
  Result.Buff := PasAI.Geometry.Low.VectorTransform(Rhs.Buff, Lhs.Buff);
end;
