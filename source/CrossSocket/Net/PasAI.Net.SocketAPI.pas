...
uses
  ...
  {$ENDIF}
  {$ELSE}
  Winapi.Windows, PasAI.Net.Winsock2, PasAI.Net.Wship6
  {$ENDIF};
...
type
  ...
  {$ELSE}
  TRawAddrInfo = PasAI.Net.Winsock2.ADDRINFOW;
  {$ENDIF}
...
begin
  ...
  Result := PasAI.Net.Winsock2.select(0, @LFDSet, nil, nil, P);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.recvfrom(ASocket, Buf, len, flags, Addr, @AddrLen);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.accept(ASocket, Addr, AddrLen);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.bind(ASocket, Addr, AddrLen);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.closesocket(ASocket);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.shutdown(ASocket, AHow);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.connect(ASocket, Addr, AddrLen);
  {$ENDIF}
  {$IFDEF DEBUG}
...
begin
  ...
  if (AServiceName <> '') then
    ...
    LService := nil;
  LRet := PasAI.Net.Wship6.getaddrinfo(LHost, LService, @AHints, @LAddrInfo);
  {$ENDIF}
...
begin
  ...
  {$ELSE}
  PasAI.Net.Wship6.freeaddrinfo(PAddrInfoW(ARawAddrInfo));
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.getpeername(ASocket, Addr, AddrLen);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.getsockname(ASocket, Addr, AddrLen);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.getsockopt(ASocket, ALevel, AOptionName, PAnsiChar(@AOptionValue), AOptionLen);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.sendto(ASocket, Buf, len, flags, Addr, AddrLen);
  {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
  Result := PasAI.Net.Winsock2.setsockopt(ASocket, ALevel, AOptionName, AOptionValue, AOptionLen);
  {$ENDIF}
end;
...
begin
  ...
  Result := Posix.SysSocket.setsockopt(ASocket, ALevel, AOptionName, AOptionValue, Cardinal(AOptionLen));
  {$ELSE}
  Result := PasAI.Net.Winsock2.setsockopt(ASocket, ALevel, AOptionName, PAnsiChar(@AOptionValue), AOptionLen);
  {$ENDIF}
end;
...
begin
  ...
  FD_ZERO(LFDSet);
  FD_SET(ASocket, LFDSet);
  Result := PasAI.Net.Winsock2.select(0, nil, @LFDSet, nil, P);
  {$ENDIF}
end;
