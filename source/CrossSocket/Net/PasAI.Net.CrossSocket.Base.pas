...
{                                                                              }
{******************************************************************************}
unit PasAI.Net.CrossSocket.Base;
...
type
  ...
  TAbstractCrossSocket = class abstract(TCore_InterfacedObject, ICrossSocket)
  protected const
    RCV_BUF_SIZE = 32768;
...
begin
  ...
  begin
    try
      // cross这套模型底层的核心是抢占策略，这里会出现多个线程从Io里面抢事件，直接从纯硬件角度挖最优效率太天马行空，太依赖硬件机制
      // 当多个线程出现并发事件，cross用的是局部锁策略，在40G高流量+E5垃圾cpu配置下，可能会小几率出现序列问题，且难以发现
