...
{ ****************************************************************************** }
unit PasAI.Sound.Bass;

{$I ..\..\PasAI.Define.inc}

interface

uses PasAI.Core, PasAI.Sound, PasAI.UnicodeMixedLib, PasAI.MediaCenter,
  PasAI.ZDB, PasAI.ZDB.ItemStream_LIB, PasAI.ZDB.HashField_LIB, PasAI.ListEngine, PasAI.PascalStrings, PasAI.UPascalStrings, PasAI.MemoryStream, PasAI.Sound.Bass.API;
...
begin
  ...
  if s.Handle <> 0 then
    begin
      ...
      RaiseInfo('Z.Sound.Bass.API error:%d', [BASS_ErrorGetCode]);
  DisposeObject(s);
end;
...
try
  ...
  if not BASS_Init(-1, 44100, 0, 0, nil) then
      RaiseInfo('Z.Sound.Bass.API init failed (%d)', [BASS_ErrorGetCode]);
  {$ELSE}
