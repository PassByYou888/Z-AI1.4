...
{ * Learn SIFT Pyramid                                                         * }
{ ****************************************************************************** }
unit PasAI.Learn.SIFT;

{$I PasAI.Define.inc}

interface

uses Math, PasAI.Core, PasAI.MemoryRaster, PasAI.Geometry2D, PasAI.UnicodeMixedLib, PasAI.DFE, PasAI.Learn.Type_LIB;

{$REGION 'Pyramid Classes Imp'}
...
type
  ...
  TPyramidCoorList = class(TCore_Object)
  private
    FList: TCore_List;
  ...
  public
    ...
    procedure SaveToDataFrame(df: TDFE);
    procedure LoadFromDataFrame(df: TDFE);

    property Items[idx: TLInt]: PPyramidCoor read GetItems; default;
  end;

  TPyramidLayer = class(TCore_Object)
  private
  ...
  public
    ...
    constructor CreateWithRasterStream(const stream: TCore_Stream); overload;
  ...
  public
    ...
    function CreateViewer: TMPasAI_Raster;
    function CreateFeatureViewer(const FeatureRadius: TGFloat; const COLOR: TPasAI_RasterColor): TMPasAI_Raster;
...
{$ENDREGION 'Pyramid Classes Imp'}
{$REGION 'Pyramid_Tool_API'}


function GF_SQR(f: TGFloat): TGFloat; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
function Diff(const f1, f2: TGFloat): TGFloat; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
function between(const idx, Start, OVER: TLInt): Boolean; overload; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
function between(const idx, Start, OVER: TGFloat): Boolean; overload; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
procedure CopySampler(var Source, dest: TGaussSpace);
procedure SamplerAlpha(const Source: TMPasAI_Raster; var dest: TGaussSpace); overload;
procedure SamplerAlpha(var Source: TGaussSpace; const dest: TMPasAI_Raster); overload;
procedure Sampler(const Source: TMPasAI_Raster; const wr, wg, wb: TGFloat; const ColorSap: TLInt; var dest: TGaussSpace); overload;
procedure Sampler(const Source: TMPasAI_Raster; var dest: TGaussSpace); overload;
procedure Sampler(var Source: TGaussSpace; const dest: TMPasAI_Raster); overload;
procedure Sampler(var Source: TGaussSpace; var dest: TBytePasAI_Raster); overload;
...
procedure ComputeSamplerSize(var width, height: TLInt); {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
function Descriptor2LVec(d: TDescriptorArray): TLVec; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}
...
function e_sqr(const sour, dest: PDescriptor): TGFloat; {$IFDEF INLINE_ASM}inline; {$ENDIF INLINE_ASM}

// feature match
function MatchFeature(const Source, dest: TFeature; var MatchInfo: TArrayMatchInfo): TLInt;
function BuildMatchInfoView(var MatchInfo: TArrayMatchInfo; const FeatureDiameter: TGFloat; const ViewFeature: Boolean): TMPasAI_Raster;

{$ENDREGION 'Pyramid_Tool_API'}
{$REGION 'Options'}
...
uses PasAI.Learn, PasAI.MemoryStream, PasAI.Status;
...
procedure SamplerAlpha(const Source: TMPasAI_Raster; var dest: TGaussSpace);
var
  i, j: TLInt;
begin
...
procedure SamplerAlpha(var Source: TGaussSpace; const dest: TMPasAI_Raster);
var
  i, j: TLInt;
begin
...
procedure Sampler(const Source: TMPasAI_Raster; const wr, wg, wb: TGFloat; const ColorSap: TLInt; var dest: TGaussSpace);
var
  i, j, c, f, w, wf: TLInt;
  r, g, b: TGFloat;
begin
  ...
  for j := 0 to Source.height - 1 do
    for i := 0 to Source.width - 1 do
      begin
        PasAI_RasterColor2F(Source.Pixel[i, j], r, g, b);
...
procedure Sampler(const Source: TMPasAI_Raster; var dest: TGaussSpace);
var
  i, j: TLInt;
  r, g, b: TGFloat;
begin
...
procedure Sampler(var Source: TGaussSpace; const dest: TMPasAI_Raster);
var
  i, j: TLInt;
begin
  dest.SetSize(length(Source[0]), length(Source));
...
procedure Sampler(var Source: TGaussSpace; var dest: TBytePasAI_Raster);
var
  i, j: TLInt;
begin
...
begin
  ...
  if ViewFeature then
    begin
      mr1 := FT1.CreateFeatureViewer(FeatureDiameter * 0.5, PasAI_RasterColorF(0.4, 0.1, 0.1, 0.5));
      mr2 := FT2.CreateFeatureViewer(FeatureDiameter * 0.5, PasAI_RasterColorF(0.4, 0.1, 0.1, 0.5));
    end
  else
    begin
  ...
  Result.SetSize(mr1.width + mr2.width, umlMax(mr1.height, mr2.height), PasAI_RasterColor(0, 0, 0, 0));
  Result.Draw(0, 0, mr1);
...
procedure TPyramidCoorList.SaveToDataFrame(df: TDFE);
var
  i: TLInt;
  p: PPyramidCoor;
  d: TDFE;
...
procedure TPyramidCoorList.LoadFromDataFrame(df: TDFE);
var
  p: PPyramidCoor;
  d: TDFE;
begin
...
procedure TPyramids.BuildLocalExtrema(const pyr_id, scale_id: TLInt; ExtremaCoorList: TCore_List);
var
  w, h: TLInt;
...
begin
  ...
  PasAI.Learn.RMatrixInverse(M, 3, Info, mRep);

  // detect a svd matrix
  Result := Info = 1;
  if Result then
      PasAI.Learn.MatrixVectorMultiply(M, 0, 2, 0, 2, False, Delta, 0, 2, 1, Offset, 0, 2, 0);
...
begin
  ...
  for i := 1 to COFFSET_DEPTH do
    begin
      ...
      dpDone := PasAI.Learn.LAbsMaxVec(Offset) < COFFSET_THRESHOLD;
      // found
      if dpDone then
          Break;
  ...
  if dog^[nowy, nowx] + PasAI.Learn.APVDotProduct(@Offset, 0, 2, @Delta, 0, 2) * 0.5 < CCONTRAST_THRESHOLD then
      Exit;
...
begin
  ...
  Result := ((PasAI.Learn.AP_Sqr(dxx + dyy) / det) > PasAI.Learn.AP_Sqr(CEDGE_RATIO + 1) / CEDGE_RATIO);
end;
...
var
  ...
  begin
    ...
    exp_denom := 2 * PasAI.Learn.AP_Sqr(gauss_weight_sigma);
      ...
      begin
        ...
        for yy := -Rad to Rad do
          begin
            ...
            if PasAI.Learn.AP_Sqr(xx) + PasAI.Learn.AP_Sqr(yy) > PasAI.Learn.AP_Sqr(Rad) then
                Continue;
            ...
            if (iBinomial > Orientation_Histogram_Binomial_Num) then
                Exit;
            Weight := Exp(-(PasAI.Learn.AP_Sqr(xx) + PasAI.Learn.AP_Sqr(yy)) / exp_denom);
...
constructor TPyramids.CreateWithRaster(const raster: TMPasAI_Raster);
var
  f: TGFloat;
  w, h: TLInt;
begin
...
begin
  raster := NewPasAI_RasterFromFile(fn);
  CreateWithRaster(raster);
  DisposeObject(raster);
end;

constructor TPyramids.CreateWithRasterStream(const stream: TCore_Stream);
var
  raster: TMPasAI_Raster;
begin
...
function TPyramids.BuildViewer(const cList: TPyramidCoorList; const radius: TGFloat; const COLOR: TPasAI_RasterColor): TMPasAI_Raster;
var
  i: TLInt;
  L: TLInt;
  invColor: TPasAI_RasterColor;
  p: PPyramidCoor;
  v1, v2: TVec2;
begin
  if length(Pyramids) = 0 then
  ...
  invColor := PasAI_RasterColorInv(COLOR);
...
class procedure TPyramids.BuildToViewer(const cList: TPyramidCoorList; const radius: TGFloat; const COLOR: TPasAI_RasterColor; const RasterViewer: TMPasAI_Raster);
var
  i: TLInt;
  L: TLInt;
  invColor: TPasAI_RasterColor;
...
class procedure TPyramids.BuildToViewer(const cList: TPyramidCoorList; const radius: TGFloat; const RasterViewer: TMPasAI_Raster);
var
  i: TLInt;
  L: TLInt;
  COLOR, invColor: TPasAI_RasterColor;
  p: PPyramidCoor;
  v1, v2: TVec2;
begin
  L := Round(radius * 2);
  ...
  for i := 0 to cList.Count - 1 do
    begin
      COLOR := PasAI_RasterColor(MT19937Rand32($FF), MT19937Rand32($FF), MT19937Rand32($FF), $FF);
      invColor := PasAI_RasterColorInv(COLOR);
...
begin
  ...
  exp_denom := 2 * PasAI.Learn.AP_Sqr(CDESC_HIST_WIDTH);
    ...
    begin
      ...
      for yy := -radius to radius do
        begin
          ...
          Weight := Exp(-(PasAI.Learn.AP_Sqr(x_rot) + PasAI.Learn.AP_Sqr(y_rot)) / exp_denom) * now_mag;
...
constructor TFeature.CreateWithRasterClipMatrix(const raster: TMPasAI_Raster; var mat: TLBMatrix);
var
  Pyramids: TPyramids;
begin
  Pyramids := TPyramids.CreateWithRaster(raster);
...
constructor TFeature.CreateWithRaster(const raster: TMPasAI_Raster);
var
  Pyramids: TPyramids;
begin
  Pyramids := TPyramids.CreateWithRaster(raster);
...
constructor TFeature.CreateWithRasterStream(const stream: TCore_Stream);
var
  Pyramids: TPyramids;
begin
...
procedure TFeature.SaveToStream(stream: TCore_Stream);
var
  i, L: TLInt;
  p: PDescriptor;
begin
...
procedure TFeature.LoadFromStream(stream: TCore_Stream);
var
  i, L: TLInt;
begin
  stream.read(L, 4);
...
function TFeature.CreateViewer: TMPasAI_Raster;
begin
  Result := NewPasAI_Raster();
  ...
  if LinkRaster <> nil then
      ...
      Result.SetSize(FWidth, FHeight, PasAI_RasterColor(0, 0, 0, $FF));
end;

function TFeature.CreateFeatureViewer(const FeatureRadius: TGFloat; const COLOR: TPasAI_RasterColor): TMPasAI_Raster;
var
  i: TLInt;
  L: TLInt;
  invColor: TPasAI_RasterColor;
