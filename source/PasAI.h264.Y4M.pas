...
uses SysUtils, PasAI.Core, PasAI.PascalStrings, PasAI.UPascalStrings, PasAI.UnicodeMixedLib, PasAI.MemoryRaster, PasAI.h264.Image_LIB, PasAI.h264.Types;
...
type
  ...
  public
    ...
    constructor CreateOnStream(const stream: TCore_Stream; const autoFreeSteam: Boolean);
  ...
  public
    ...
    *)
    constructor CreateOnStream(const w, h, PSF: uint16_t; const stream: TCore_Stream);
...
begin
  ...
  if not CompareMemory(@token[0], @Y4M_Token[0], 9) then
      RaiseInfo('Not a Z.h264.Y4M format');
...
constructor TY4MReader.CreateOnStream(const stream: TCore_Stream; const autoFreeSteam: Boolean);
begin
  inherited Create;
  FFrameCount := 0;
  FCurrentFrame := 0;
...
begin
  ...
  if not CompareMemory(@magic[0], @FRAME_MAGIC[0], FRAME_MAGIC_SIZE) then
      RaiseInfo('Not a Z.h264.Y4M Frame');
...
constructor TY4MWriter.CreateOnStream(const w, h, PSF: uint16_t; const stream: TCore_Stream);
var
  s: TPascalString;
  b: TBytes;
begin
...
procedure TY4MWriter.WriteFrame(raster: TMPasAI_Raster);
var
  FrameSize: int32_t;
begin
